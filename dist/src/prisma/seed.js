...
const seed = async () => {
  ...
  `, [password]);
    const adminRes = await database_1.pool.query(`SELECT id FROM users WHERE email = 'admin@finance.com'`);
...
seed().catch((err) => { logger_1.logger.error('Seed failed', { err }); process.exit(1); });
